...
const DeletePackageModal = ({ isOpen, onConfirm, onClose, packageIdToDelete,packages }) => {
    console.log('Delete Package Modal', packages)
    return (
      <div className={`fixed inset-0 z-50 overflow-y-auto bg-gray-500 bg-opacity-75 px-4 py-6 sm:px-0 sm:py-8 ${!isOpen && 'hidden'}`}>
        <div className="w-full max-w-sm mx-auto bg-white shadow-md rounded-lg overflow-hidden">
          ...
          <div className="modal-body p-6 text-center">
            {packageIdToDelete && ( // Check if packageIdToDelete exists before displaying
              <p className="text-sm text-gray-700">
                Are you sure you want to delete package : {packages?.package_name}?
              </p>
            )}
          </div>
