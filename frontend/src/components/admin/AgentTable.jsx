...
import { handleFetchAgents } from '../../redux/reducers/admin/adminReducer';
import { FaSpinner } from 'react-icons/fa';

const AgentListRow = ({ email, first_name, last_name, phone }) => {
    ...
    <tr className="hover:bg-gray-50">
      <th className="flex gap-3 px-6 py-4 font-normal text-gray-900">
        ...
        <div className="text-sm">
          <div className="font-medium text-gray-700">{first_name.concat(" " + last_name)}</div>
...
export default function AgentTable() {
  ...
  useEffect(() => {
    if (agents && agents.agent) {
      setFilteredAgents(agents.agent);
    }
  }, [agents]);
  ...
  const handleSearch = (e) => {
    ...
    if (query.trim() === '') {
      ...
      const filtered = agents.agent.filter(agent =>
        ...
        {!agents || agents.length === 0 ? (
          ...
          <table className="w-full border-collapse bg-white text-left text-sm text-gray-500">
            <thead className="bg-gray-50">
              <tr>
                <th scope="col" className="px-6 py-4 font-medium text-gray-900">Agent Name</th>
            ...
            <tbody className="divide-y divide-gray-100 border-t border-gray-100">
              {filteredAgents.map(agent => (
                <AgentListRow key={agent.id} {...agent} />
              ))}
            </tbody>
          </table>
        )}
